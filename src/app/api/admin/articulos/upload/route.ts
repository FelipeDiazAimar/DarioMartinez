...
export async function POST(request: Request) {
  try {
    ...
    if (typeof articleId !== 'string' || !articleId.trim()) {
      return NextResponse.json({ error: 'El campo articleId es obligatorio.' }, { status: 400 });
    }
    ...
    const cleanArticleId = articleId.trim().replace(/[^a-zA-Z0-9_-]/g, '') || 'articulo';
    const filePath = `articulos/${cleanArticleId}-${Date.now()}.${ext}`;
    ...
    await supabase
      ...
      .insert({
        ...
        seccion: 'articulos',
        etiqueta: `articulo-${cleanArticleId}`,
      });
